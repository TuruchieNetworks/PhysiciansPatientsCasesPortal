...
<body class="container-fluid p-8" style="
    ...
    <table class="table table-dark text-center" style="border-radius: 5%;">
		...
		<tbody>
		    <tr style="text-decoration:none; color:aqua">
		        ...
		        <td><a href="/mellowHealth/patientsPortal/patients/${onePatient.id}" style="text-decoration:none; color:aqua"><c:out value="${onePatient.currentChiefComplaint}" /></a></td>
	            ...
	            <c:when test="${patient.hasTravelledOutsideTheUnitedStatesForMoreThan30Days == 'No' || patient.patientCases.size() % 2 == 0}">
	             ...
	             <c:otherwise>
	             	...
	             	margin:10px 0; width: 100%; display:block; padding: 10px"  href="/mellowHealth/patientsPortal/patients/${onePatient.id}" class="btn btn-success">MELLOW HEALTH VISITS: <c:out value="${onePatient.patientCases.size()}"/> -Following <c:out value="${onePatient.physiciansPatients.size()}"/> PRIVATE PHYISICIANS!</a></h1>
				 </c:otherwise>
			     </c:choose>
	    	...
	    	<div class="form-group"style="display:flex; justify-content:space-between;align-items:center;
				<c:choose>
	             ...
	             border-radius:5%;padding:5px;">
				    <a href="/mellowHealth/patientsPortal/patients/${onePatient.id}" style="text-decoration:none;text-align:center; width:100%;">
				    	<h1 style="font-family:fantasy;background:rgba(18, 12, 11, 0.7);border-radius:5%;padding:5px;">${onePatient.patientFirstName} Mellow Health Details: <c:out value="${onePatient.patientCases.size()} Patient Cases- Primary Physicians: ${onePatient.physiciansPatients.size()}"/></h1></a>
				   </div>
	...
	<div class="form-group"style="
		...
		<h1 style=" margin: 0px 15px 0 0; width: 100%"><a href="/mellowHealth/hospitalDashboard/patientCases/newPatientCase" class="btn btn-primary" style="
	    		<c:choose>
	            <c:when test="${onePatient.gender == 'Male' || onePatient.race == 'Black'}">
			         color: rgb(412, 580, 515); background: rgba(0.531, 0.64, 16, 0.9); 
	            </c:when>
		...
		<h1 style=" margin: 0px 15px 0 0; width: 100%"><a href="/mellowHealth/patientsPortal/patients" class="btn btn-warning" style="
			    ...
			    <c:choose>
			        ...
			        <c:when test="${onePatient.id == loggedInPatient.id and onePatient.patientCases.size() == 1}">
				     ...
				     <c:otherwise>
							<input type="submit" value="${onePatient.patientCases.size()}: Patient Cases!" class="btn btn-success border-bottom border-2"style="
					    		<c:choose>
					            <c:when test="${onePatient.gender == 'Male' || onePatient.race == 'Black'}">
							         color: rgb(412, 580, 515);background: rgba(13, 64, 60, 0.9); 
					            </c:when>
