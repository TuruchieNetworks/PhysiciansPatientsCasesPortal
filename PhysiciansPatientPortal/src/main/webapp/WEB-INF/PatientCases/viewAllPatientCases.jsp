		    ...
		    <tr style="text-decoration:none; color:aqua">
	            ...
	            <c:when test="${createdAt.contains('Thur') || createdAt.contains('Fri')}">
	             ...
	             <c:otherwise>
	             	...
	             	margin:10px 0; width: 100%; display:block; padding: 10px"  href="/mellowHealth/physicians/${loggedInPhysician.id}" class="btn btn-success"><c:out value="${loggedInPhysician.patientCases.size()}" /> PATIENT CASES- <c:out value="${loggedInPhysician.physiciansPatients.size()}" /> PATIENTS IN CARE!</a></h1>
				 </c:otherwise>
			     </c:choose>
	    	<div class="form-group"style="display:flex; justify-content:space-between;align-items:center;
				<c:choose>
				 ...
				 <a href="/mellowHealth/patientsPortal/patients/${patientCase.patient.id}" style="text-decoration:none;text-align:center; width:100%;">
					 <h1 style="font-family:fantasy;background:rgba(18, 12, 11, 0.7);border-radius:5%;padding:5px;">Total Patient Cases In Your Care: ${loggedInPhysician.patientCases.size()}- ${loggedInPhysician.physiciansPatients.size()} Private Patient Cases in Dr. ${loggedInPhysician.firstName}</h1>
				 </a>
		   </div>
    	<table class="table table-dark text-center" style="border-radius: 5%;">
	    <thead>
		...
		<div style="text-decoration:none;color:aqua;display:flex;justify-content:space-between;align-items:center;padding:10px;margin:5px;border-radius:5%;">
			...
			<c:forEach items="${allPatientCases}" var="patientCase">
		     ...
		     <div class="btn btn-outline-warning" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:0 10px; padding:10px; background:rgba(1.33, 0.64, 30.60, 0.9);">
              ...
              <td>
		        <div class="btn btn-outline-warning" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:0 10px; padding:10px; background:rgba(1.33, 0.64, 30.60, 0.9);">
		          <p class="btn btn-outline-success" style="text-align:center;">
	   		  ...
	   		  <td>
		        <div class="btn btn-outline-warning" style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;text-align:center;margin:0 10px; padding:10px; background:rgba(1.33, 0.64, 30.60, 0.9);">
			        <p class="btn btn-outline-warning" style="text-align:center;background:rgba(0.1, 30.1, 32, 0.9);">Follow Up: 
